...
HEADERS={'User-Agent':'Mozilla/5.0 (compatible; DomesticSaleMVP/0.5; +https://github.com/musaonowaka/domestic-sale-site)','Accept':'text/html,application/json;q=0.9,*/*;q=0.8'}
...
def product_image(url):
    # Shopify exposes a lightweight product JSON endpoint. Prefer it because
    # it is more consistent than theme-specific lazy-loaded HTML.
    try:
        r=requests.get(url+'.js',headers=HEADERS,timeout=20)
        if r.ok:
            data=r.json()
            featured=data.get('featured_image')
            u=normalize_image(featured)
            if u: return u
            images=data.get('images') or []
            if images:
                u=normalize_image(images[0])
                if u: return u
    except Exception:
        pass
    # Fallback for products where the JSON endpoint is unavailable.
    try:
        r=requests.get(url,headers=HEADERS,timeout=20)
        r.raise_for_status()
        soup=BeautifulSoup(r.text,'html.parser')
        for sel in ('meta[property="og:image:secure_url"]','meta[property="og:image"]','meta[name="twitter:image"]','link[rel="image_src"]'):
            node=soup.select_one(sel)
            if node:
                u=normalize_image(node.get('content') or node.get('href'))
                if u: return u
        # JSON-LD often contains the canonical product image even when meta tags differ.
        import json
        for node in soup.select('script[type="application/ld+json"]'):
            try:
                ...
                for obj in candidates:
                    ...
                    if '@graph' in obj and isinstance(obj['@graph'],list): candidates.extend(obj['@graph'])
                    if str(obj.get('@type','')).lower()=='product':
                        imgs=obj.get('image') or []
                        if not isinstance(imgs,list): imgs=[imgs]
                        if imgs:
                            u=normalize_image(imgs[0])
                            if u: return u
            except Exception:
                continue
    except Exception:
...
def enrich_images(items):
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futures):
            try: futures[f]['image']=f.result()
            except Exception: futures[f]['image']=''
    return items
...
def fetch():
    ...
    for page in range(1,31):
        ...
        time.sleep(1.0)
    return enrich_images(list(found.values()))
...
def build(items):
    ...
    for x in items:
        ...
        if x.get('image'):
            visual=f'<a class="visual" href="{esc(x["url"])}" target="_blank" rel="noopener sponsored"><img src="{esc(x["image"])}" alt="{esc(x["brand"]+" "+x["title"])}" loading="lazy" referrerpolicy="no-referrer" onerror="this.parentElement.classList.add(\'broken\');this.remove()"></a>'
        else: visual='<div class="visual broken"></div>'
